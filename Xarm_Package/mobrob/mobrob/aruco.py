        ...
        while True: 
            ...
            if (len(corners) > 0):
                ...
                for i in range(0,len(corners)):
                    ...
                    # Extract the integer ID of the current tag
                    tag_id = ids[i][0]
                    
                    # Dynamically set the marker size based on the ID
                    if tag_id == 2:
                        current_marker_length = 0.060325  # <--- REPLACE with actual Big Tag (Amp) width in meters
                    elif tag_id == 4:
                        current_marker_length = 0.028575  # <--- REPLACE with actual Little Tag (Robot) width in meters
                    else:
                        current_marker_length = 0.061  # Default fallback
                    
                    # Estimate the 3D location of the marker using the correct physical size
                    rot_vec, trans_vec, marker_points = aruco.estimatePoseSingleMarkers(
                        corners[i], current_marker_length, cam_matrix, dist_matrix
                    )
                    ...
                    toSend = toSend +  "ID:%d,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f " % (
                        ids[i], trans_vec[0][0][0], trans_vec[0][0][1], trans_vec[0][0][2], 
                        rot_vec[0][0][0], rot_vec[0][0][1], rot_vec[0][0][2]
                    )
