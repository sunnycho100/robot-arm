...
"""ArUco tag pose from the C270, headless.
  ...
  see.py [--size=0.061] [--frames=30] [--shot=/home/pi/tag.jpg]
...
CALIB = '/home/pi/cv'
TAG_SIZE = 0.061        # default tag edge, meters. Distance scales directly with it.
    ...
    for _ in range(frames):
        ...
        for c, i in zip(corners, ids.flatten()):
            rvec, tvec, _ = aruco.estimatePoseSingleMarkers(
                c, size, _cam_matrix, _dist_matrix)
...
def main():
    ...
    if not tags:
        sys.exit('no tags found. Check lighting, focus, that the whole tag plus '
                 'its white border is inside the frame, and that it is DICT_6X6_250.')
